...
def build_dataloaders(args: CFG, tokenizer: AutoTokenizer):
    ...
    load_train_data = load_dataset(args.train_dataset_name, args.train_dataset_config_name,
                                   split=args.choose_train_split)
    ...
    load_eval_data = load_dataset(args.eval_dataset_name, args.eval_dataset_config_name,
                                  split=args.choose_eval_split)
    ...
    tokenized_train_dataset = shuffled_train_files.map(tokenize, batched=True,
                                                       remove_columns=[args.select_input_string])
    ...
    train_dataloader = DataLoader(tokenized_train_dataset, shuffle=True, sampler=sample_train_dataset, drop_last=True,
                                  collate_fn=default_data_collator, batch_size=args.batch_size)

    # Create the validation dataloader. If the length of a tokenized input sequence is less than 2048 drop it.
    eval_dataloader = DataLoader(tokenized_eval_dataset, sampler=sample_eval_dataset, drop_last=True,
                                 collate_fn=default_data_collator, batch_size=args.batch_size)
